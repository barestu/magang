<?php
class Laporan_pegawai_diklat_model extends CI_Model {
...
if (!empty($where)) {
 $query = $this->db->query("SELECT nip, nama, nama_dik, jenis_dik, tahun_dik
FROM tbl_pegawai NATURAL JOIN tb_jabatan NATURAL JOIN tb_bidang NATURAL JOIN tb_direktorat NATURAL JOIN tb_diklat
WHERE tbl_pegawai.id_bid = tb_bidang.id_bid 
AND tbl_pegawai.id_direktorat = tb_direktorat.id_direktorat 
AND tbl_pegawai.id_peg = tb_diklat.id_peg 
...
if (!empty($where)) {
 $query = $this->db->query("SELECT nip, nama, nama_dik, jenis_dik, tahun_dik, nama_direktorat, nama_jab, nama_bid
FROM tbl_pegawai NATURAL JOIN tb_jabatan NATURAL JOIN tb_bidang NATURAL JOIN tb_direktorat NATURAL JOIN tb_diklat 
WHERE tbl_pegawai.id_bid = tb_bidang.id_bid 
AND tbl_pegawai.id_direktorat = tb_direktorat.id_direktorat 
AND tbl_pegawai.id_peg = tb_diklat.id_peg  
